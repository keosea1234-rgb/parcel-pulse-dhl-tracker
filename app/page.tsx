...
type BatchItem = { trackingNumber: string; state: QueueState; tracking?: TrackingResult; error?: string };
...
export default function Home() {
  ...
  async function downloadExcel() {
    ...
    const summaryRows = [{
      ...
      "Data source": "DHL Unified Tracking",
    }];
  ...
  async function runBatch(event: FormEvent<HTMLFormElement>) {
    ...
    for (const [index, trackingNumber] of trackingNumbers.entries()) {
      ...
      try {
        ...
        failedCount += 1;
        updateItem(index, { state: "failed", error: error instanceof Error ? error.message : "Tracking request failed" });
      }
    ...
    <main className="app-shell">
      ...
      <section className="batch-layout" aria-live="polite">
        <article className="batch-card">
          ...
          </div></div> : <div className="empty-state"><span className="empty-count">DHL</span><h3>Ready for live tracking</h3><p>Paste one or more live DHL tracking numbers to retrieve their current DHL status and shipment details.</p></div>}
      ...
      <section className="integration-strip"><p><span className="pulse-dot" /> LIVE DHL UNIFIED TRACKING</p><span>Real shipment responses only</span><span className="strip-divider" /><span>One request every 5 seconds to respect DHL&apos;s development rate limit</span></section>
    </main>
  );
}
